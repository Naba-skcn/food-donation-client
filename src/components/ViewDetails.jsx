...
const ViewDetails = () => {
    ...
    const requestData ={
        foodId, donatorEmail, additionalNotes
    }

        try {
            const { data } = await axios.put(`http://localhost:5000/food/${product._id}`, requestData);
            console.log(data);
            Swal.fire({
                icon: 'success',
                title: 'Food Added!',
    ...
    const handleRequest = async () => {
        try {
            // Prepare request data including additional notes and donor email
            const requestData = {
                additionalNotes: additionalNotes,
                donatorEmail: user ? user.email : userEmail,
                requestDate: new Date().toLocaleString() 
            };
            
            console.log(requestData);
    
            // Send a PUT request to update the food status, additional notes, and donor email
            const response = await axios.put(`http://localhost:5000/food/${id}/request`, requestData);
...
<input id="requestDate" name='requestDate' type="text" value={new Date().toLocaleString()} readOnly />
                            ...
                            <label htmlFor="additionalNotes" className="text-xs text-white bg-green-900 p-1 rounded-lg">Additional Notes</label>
                                        <div className="col-span-full">
                                            <textarea id="additionalNotes" name='additionalNotes' defaultValue={product.additionalNotes} onChange={handleInputChange} placeholder="Additional Notes" className="w-full rounded-md focus:ring focus:ring-opacity-75 dark:text-gray-50 focus:dark:ring-violet-600 dark:border-gray-300"></textarea>
                                        </div>
                                        ...
                                        <div className="modal-action col-span-full">
                                            <label onClick={handleRequest} htmlFor="my_modal_8" className="btn btn-block text-white rounded-md bg-[#66A000] border-[#66A000]">Request</label>
                                        </div>
                                </div>
                            </div>
                        </div>
